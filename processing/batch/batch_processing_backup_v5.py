...
def main():
    ...
    sc.setLogLevel("WARN")
    #load location file
    locdata = sc.textFile("s3n://checkin0129/checkins_new.csv").map(lambda x: x[1])
    ...
    countsloc = locdata.map(lambda line: line.split('|'))      
   # line: (u123, u234)
	# out: u123, u234
   ...
   # Processed_data = counts.map(test2).map(lambda x: (x[1][0], (x[1][0], x[1][1], x[1][2]))).reduceByKey(lambda x,y: (x[0],x[1],x[2]+y[2]))
    ...
    loc_data = countsloc.map(test2).map(lambda x: (x[1][0], (x[1][0], x[1][1], x[1][2]))).reduceByKey(lambda x,y: (x[0],x[1],x[2])).take(10)
